...
CZfuchaStake::CZfuchaStake(const libzerocoin::CoinSpend& spend)
{
...
int CZfuchaStake::GetChecksumHeightFromMint()
{
...
int CZfuchaStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZfuchaStake::GetChecksum()
{
    return nChecksum;
}

// The zfucha block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZfuchaStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZfuchaStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zfucha staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZfuchaStake::GetUniqueness()
{
    //The unique identifier for a zfucha is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZfuchaStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZfuchaStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zfucha that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZfuchaOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zfucha output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zfucha", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZfuchaOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zfucha output", __func__);
...
bool CZfuchaStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZfuchaStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzfuchaTracker* zfuchaTracker = pwallet->zfuchaTracker.get();
    CMintMeta meta;
    if (!zfuchaTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zfuchaTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!fucha Stake
bool CfuchaStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CfuchaStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CfuchaStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CfuchaStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CfuchaStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CfuchaStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CfuchaStake::GetUniqueness()
{
    //The unique identifier for a fucha stake is the outpoint
...
CBlockIndex* CfuchaStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
