...
#include "qt/fucha/topbar.h"
#include "qt/fucha/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/fucha/lockunlock.h"
#include "qt/fucha/qtutils.h"
#include "qt/fucha/receivedialog.h"
...
TopBar::TopBar(fuchaGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopfucha, ui->labelAmountTopzfucha}, "amount-small-topbar");
    setCssProperty({ui->labelAmountfucha, ui->labelAmountzfucha}, "amount-topbar");
    setCssProperty({ui->labelPendingfucha, ui->labelPendingzfucha, ui->labelImmaturefucha, ui->labelImmaturezfucha}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzfucha->setVisible(false);
    ui->labelAmountzfucha->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzfucha->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezfucha->setVisible(false);
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount fuchaAvailableBalance = balance + delegatedBalance;

    // zfucha Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalfucha = GUIUtil::formatBalance(fuchaAvailableBalance, nDisplayUnit);
    QString totalzfucha = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopfucha->setText(totalfucha);
    ui->labelAmountTopzfucha->setText(totalzfucha);

    // Expanded
    ui->labelAmountfucha->setText(totalfucha);
    ui->labelAmountzfucha->setText(totalzfucha);

    ui->labelPendingfucha->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzfucha->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmaturefucha->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezfucha->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
