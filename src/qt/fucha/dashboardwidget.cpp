...
#include "qt/fucha/dashboardwidget.h"
#include "qt/fucha/forms/ui_dashboardwidget.h"
#include "qt/fucha/sendconfirmdialog.h"
#include "qt/fucha/txrow.h"
...
DashboardWidget::DashboardWidget(fuchaGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of fucha staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarefucha, "square-chart-fucha");
    setCssProperty(ui->labelSquarezfucha, "square-chart-zfucha");
    setCssProperty(ui->labelfucha, "text-chart-fucha");
    setCssProperty(ui->labelZfucha, "text-chart-zfucha");
    ui->labelZfucha->setVisible(false);
    ui->labelSquarezfucha->setVisible(false);
    ...
    ui->labelAmountZfucha->setText("0 zfucha");
    ui->labelAmountZfucha->setVisible(false);
    ui->labelAmountfucha->setText("0 fucha");
    setCssProperty(ui->labelAmountfucha, "text-stake-fucha-disable");
    setCssProperty(ui->labelAmountZfucha, "text-stake-zfucha-disable");
    ...
    ui->btnHowTo->setText(tr("How to get fucha"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("fucha")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isfucha = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZfucha;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair fucha, zfucha
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            fucha = (pair.first != 0) ? pair.first / 100000000 : 0;
            zfucha = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalfucha += pair.first;
            chartData->totalZfucha += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesfucha.append(fucha);
        chartData->valueszfucha.append(zfucha);

        int max = std::max(fucha, zfucha);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("fucha");
    set1 = new QBarSet("zfucha");
    ...
    set0->append(chartData->valuesfucha);
    set1->append(chartData->valueszfucha);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalfucha > 0 || chartData->totalZfucha > 0) {
        setCssProperty(ui->labelAmountfucha, "text-stake-fucha");
        setCssProperty(ui->labelAmountZfucha, "text-stake-zfucha");
    } else {
        setCssProperty(ui->labelAmountfucha, "text-stake-fucha-disable");
        setCssProperty(ui->labelAmountZfucha, "text-stake-zfucha-disable");
    }
    forceUpdateStyle({ui->labelAmountfucha, ui->labelAmountZfucha});
    ui->labelAmountfucha->setText(GUIUtil::formatBalance(chartData->totalfucha, nDisplayUnit));
    ui->labelAmountZfucha->setText(GUIUtil::formatBalance(chartData->totalZfucha, nDisplayUnit, true));
