...
#include "qt/fucha/send.h"
#include "qt/fucha/forms/ui_send.h"
#include "qt/fucha/addnewcontactdialog.h"
#include "qt/fucha/qtutils.h"
#include "qt/fucha/sendchangeaddressdialog.h"
#include "qt/fucha/optionbutton.h"
#include "qt/fucha/sendconfirmdialog.h"
#include "qt/fucha/myaddressrow.h"
#include "qt/fucha/guitransactionsutils.h"
...
#include "zfuchacontroldialog.h"

SendWidget::SendWidget(fuchaGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("fucha");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zfucha");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (fucha)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a fucha address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 fucha");
    ...
    setCssProperty(coinIcon, "coin-icon-fucha");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onfuchaSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onfuchaSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send fucha");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zfucha");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZfucha = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZfucha));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZfucha ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendfucha = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendfucha ? AskPassphraseDialog::Context::Send_fucha : AskPassphraseDialog::Context::Send_zfucha, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendfucha) ? send(recipients) : sendZfucha(recipients)) {
...
bool SendWidget::sendZfucha(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zfucha is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zfucha selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZfuchaControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZfuchaControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZfucha(
        ...
        inform(tr("zfucha transaction sent!"));
        ZfuchaControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == Zfucha_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zfucha require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zfucha transaction
        ...
        emit message("zfucha transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isfucha){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any fucha to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZfuchaControlDialog *zfuchaControl = new ZfuchaControlDialog(this);
            zfuchaControl->setModel(walletModel);
            zfuchaControl->exec();
            ui->btnCoinControl->setActive(!ZfuchaControlDialog::setSelectedMints.empty());
            zfuchaControl->deleteLater();
        } else {
            inform(tr("You don't have any zfucha in your balance to select."));
        }
    }
}
...
void SendWidget::onfuchaSelected(bool _isfucha){
    isfucha = _isfucha;
    setCssProperty(coinIcon, _isfucha ? "coin-icon-fucha" : "coin-icon-zfucha");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress fuchaAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(fuchaAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(fuchaAdd.Get(), dialog->getLabel().toStdString(), "send")) {
