...
#include "qt/fucha/privacywidget.h"
#include "qt/fucha/forms/ui_privacywidget.h"
#include "qt/fucha/qtutils.h"
#include "guiutil.h"
#include "qt/fucha/denomgenerationdialog.h"
#include "qt/fucha/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(fuchaGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zfucha anonymizes your fucha by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zfucha or convert back to fucha"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 fucha ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zfucha");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zfucha");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zfucha");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zfucha");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zfucha");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zfucha");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zfucha");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zfucha");
    ...
    // Only Convert to fucha enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzfucha->setTitleClassAndText("btn-title-grey", "Total 0 zfucha");
    ui->btnTotalzfucha->setSubTitleClassAndText("text-subtitle", "Show denominations of zfucha owned.");
    ui->btnTotalzfucha->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select fucha outputs to mint into zfucha.");
    ...
    connect(ui->btnTotalzfucha, SIGNAL(clicked()), this, SLOT(onTotalZfuchaClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zfucha");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of fucha to mint into zfucha"));
    }else{
        btnText = tr("Convert back to fucha");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zfucha to convert back into fucha"));
    }
...
void PrivacyWidget::onTotalZfuchaClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzfucha->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzfucha->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zfucha is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zfucha, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zfucha").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zfucha minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZfucha(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zfucha converted back to fucha"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zfucha </b>";
    ...
    ui->btnTotalzfucha->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
