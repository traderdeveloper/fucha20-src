...
#include "qt/fucha/forms/ui_addresseswidget.h"
#include "qt/fucha/addresslabelrow.h"
#include "qt/fucha/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(fuchaGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a fucha address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress fuchaAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(fuchaAdd.Get(), label.toUtf8().constData(),
                fuchaAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
