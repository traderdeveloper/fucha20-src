...
#include "ui_zfuchacontroldialog.h"
...
std::set<std::string> ZfuchaControlDialog::setSelectedMints;
std::set<CMintMeta> ZfuchaControlDialog::setMints;

bool CZfuchaControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZfuchaControlDialog::COLUMN_DENOMINATION || column == ZfuchaControlDialog::COLUMN_VERSION || column == ZfuchaControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZfuchaControlDialog::ZfuchaControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZfuchaControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zfucha Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZfucha->setProperty("cssClass", "text-main-purple");
    ui->labelZfucha_int->setProperty("cssClass", "text-main-purple");
...
ZfuchaControlDialog::~ZfuchaControlDialog()
{
    delete ui;
}

void ZfuchaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZfuchaControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZfuchaControlWidgetItem* itemDenom(new CZfuchaControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZfuchaControlWidgetItem *itemMint = new CZfuchaControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zfuchaTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zfuchaTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zfucha.";
            else if (!mint.isSeedCorrect)
                strReason = "The zfucha seed used to mint this zfucha is not the same as currently hold in the wallet";
            else
...
void ZfuchaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZfuchaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZfucha_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZfuchaControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZfuchaControlDialog::GetSelectedMints()
{
...
void ZfuchaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
