...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::fucha);
    ...
    if (settings.contains("nAnonymizefuchaAmount") || reset)
        SoftSetArg("-anonymizefuchaamount", settings.value("nAnonymizefuchaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fucha.cpp during shotdown to also remove the window positions
