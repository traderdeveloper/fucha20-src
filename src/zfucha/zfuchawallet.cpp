...
CzfuchaWallet::CzfuchaWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfucha seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZfuchaSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZfuchaSeed_deprecated()) {
                    LogPrintf("%s: Updated zfucha seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfucha seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfucha wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzfuchaWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZfuchaCount(nCountLastUsed);
    else if (!walletdb.ReadZfuchaCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzfuchaWallet::Lock()
{
    seedMaster = 0;
}

void CzfuchaWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzfuchaWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZfucha(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzfuchaWallet::LoadMintPoolFromDB()
{
...
void CzfuchaWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzfuchaWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzfuchaWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfuchaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzfuchaWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZfucha(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfuchaTracker which also adds to database
    pwalletMain->zfuchaTracker->Add(dMint, true);
...
void CzfuchaWallet::SeedToZfucha(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzfuchaWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzfuchaWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZfuchaCount(nCountLastUsed);
...
void CzfuchaWallet::GenerateDeterministicZfucha(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzfuchaWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzfuchaWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzfuchaWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
