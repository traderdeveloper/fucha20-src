...
CzfuchaTracker::CzfuchaTracker(std::string strWalletFile)
{
...
CzfuchaTracker::~CzfuchaTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzfuchaTracker::Init()
{
...
bool CzfuchaTracker::Archive(CMintMeta& meta)
{
...
bool CzfuchaTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzfuchaTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzfuchaTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzfuchaTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzfuchaTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzfuchaTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzfuchaTracker::GetSerialHashes()
{
...
CAmount CzfuchaTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzfuchaTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzfuchaTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzfuchaTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzfuchaTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzfuchaTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzfuchaTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzfuchaTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzfuchaTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzfuchaTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzfuchaTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzfuchaWallet* zfuchaWallet)
{
    bool iszfuchaWalletInitialized = (NULL != zfuchaWallet);
    CMintMeta meta;
    ...
    if (! iszfuchaWalletInitialized)
        zfuchaWallet = new CzfuchaWallet(strWalletFile);
    meta.isSeedCorrect = zfuchaWallet->CheckSeed(dMint);
    if (! iszfuchaWalletInitialized)
        delete zfuchaWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzfuchaTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzfuchaTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzfuchaTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzfuchaTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzfuchaTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzfuchaTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzfuchaWallet* zfuchaWallet = new CzfuchaWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zfuchaWallet);
        }
        delete zfuchaWallet;
        LogPrint("zero", "%s: added %d dzfucha from DB\n", __func__, listDeterministicDB.size());
    }
