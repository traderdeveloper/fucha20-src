...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight >   4400000) return     0.5 * COIN;
    if (nHeight >   1900000) return     1 * COIN;
    if (nHeight >    900000) return     3  * COIN;
    if (nHeight >    150000) return     6 * COIN;
    if (nHeight >         2) return     7 * COIN;
    if (nHeight >         1) return     9000000 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 FUCHA tx -> 1 confirmation
