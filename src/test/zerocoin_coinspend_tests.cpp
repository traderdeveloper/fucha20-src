...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzfuchaWallet *czfuchaWallet = new CzfuchaWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czfuchaWallet->GenerateDeterministicZfucha(denom, coin, dMint, true);
        czfuchaWallet->UpdateCount();
