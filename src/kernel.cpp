...
bool GetHashProofOfStake(const CBlockIndex* pindexPrev, CStakeInput* stake, const unsigned int nTimeTx, const bool fVerify, uint256& hashProofOfStakeRet) {
    ...
    if (fVerify) {
        LogPrint("staking", "%s :{ nStakeModifier=%s\n"
                            ...
                            "}\n",
            __func__, HexStr(modifier_ss), ((stake->IsZfucha()) ? "Not available" : std::to_string(stake->getStakeModifierHeight())));
    }
    return true;
}
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s : zfucha stake block is less than allowed start height", __func__);

    if (CZfuchaStake* zfucha = dynamic_cast<CZfuchaStake*>(stake)) {
        CBlockIndex* pindexFrom = zfucha->GetIndexFrom();
        if (!pindexFrom)
            return error("%s : failed to get index associated with zfucha stake checksum", __func__);

        int depth = (nPreviousBlockHeight + 1) - pindexFrom->nHeight;
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : zfucha stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zfucha->GetValue()));
        if (nChecksum200 != zfucha->GetChecksum())
            return error("%s : accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zfucha->GetChecksum(), nChecksum200);
    } else {
...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZfuchaStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zfucha fails context checks", __func__);
    } else {
        ...
        CfuchaStake* fuchaInput = new CfuchaStake();
        fuchaInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(fuchaInput);
    }
    return true;
}
