...
*Setup instructions for a gitian build of fucha Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the fucha
...
to the fucha GitHub release page.
...
- [Building fucha Coin](#building-fucha-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/fucha-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/fucha-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/fucha-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/fucha-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/fucha-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/fucha-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/traderdeveloper/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@fucha.rocks) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/fucha-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/fucha-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/fucha-osx-*-build.assert
```
