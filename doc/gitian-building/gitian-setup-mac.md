...
The build script we'll be using is contained in the fucha github repository ([contrib/gitian-build.py](https://github.com/traderdeveloper/fucha/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the fucha repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/fucha/fucha/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, fucha-detached-sigs, and fucha GitHub repos.
