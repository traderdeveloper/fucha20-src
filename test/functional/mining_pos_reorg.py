...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected FUCHA and zFUCHA supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zFUCHAsupply']:
                assert_equal(node['zFUCHAsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check FUCHA and zFUCHA supply at the beginning
        # ------------------------------------------
        # zFUCHA supply: 2 coins for each denomination
        ...
        # FUCHA supply: block rewards minus burned fees for minting
        ...
        # Verify that FUCHA and zFUCHA supplies were properly updated after the spends and reorgs
        self.log.info("Check FUCHA and zFUCHA supply...")
